<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        if (empty($name)){
            $err = true;
            $result = "Enter pupil's full name";
        }

        if (empty($gender)){
            $err = true;
            $result = "Select the pupil's gender";
        }

        if (empty($class)){
            $err = true;
            $result = "Select the pupil's class";
        }
        ...
        if (empty($gName)){
            $err = true;
            $result = "Enter the name of the pupil's guardian";
        }
        if (empty($gNum)){
            $err = true;
        ...
        if (!$err){
            $code = $Model->getToken(6);
            $stddata = [strToUpper($name), $gender,$dob, strToUpper($pob), strToUpper($gName), $gNum, $gEmail, $gAddress, $code, strToUpper($motherName), strToUpper($fatherName), $admNum, $section];
            $result = $Primodel->RegisterNewPupil($stddata);
...
<div class="row">
    ...
    <div class="col-xs-8">
        <label id="label1"><?= $lang[$_SESSION['lang']]["RegisterNewPupil"] ?></label>
