...
$term_name = strtoupper($Model->GetTermName($exam_id));
$page_title = $terms[$term_name]." - Evaluation ".$Model->GetSequenceName($exam_id)." - ".utf8_decode($lang[$_SESSION['lang']]["resultSummary"])." - ".$Model->GetAClassName($class_id);
$student_codes = [];
$data = []; $students_totals = []; $passed=0; $percentPass = 0;

$averages = $Model->GetStudentAverages($exam_id, $class_id, $year_id);
$positions = $Model->GetSequencePosition($year_id, $class_id, $exam_id);
...
//foter page
    ...
    $pdf->Cell(30, 7, $lang[$_SESSION['lang']]["ClassAv"].round($Model->GetClassAverage($exam_id, $class_id, $year_id), 2), 0);
    $pdf->Ln();
    $pdf->Cell(22,7,$lang[$_SESSION['lang']]["Rank"],1);
    $pdf->Cell(95,7,$lang[$_SESSION['lang']]["Name"],1);
    ...
    foreach($averages as $student_code => $average){
        ...
        $pdf->Cell(95,7,$s[0]['name'],1);
        $pdf->Cell(20,7,$s[0]['gender'],1);
        $pdf->Cell(20,7,$average,1);
        $pdf->Cell(35,7,$Model->NumberOfPapersSeq($student_code, $year_id, $class_id, $exam_id),1);
    }
