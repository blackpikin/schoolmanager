...
function Header()
    ...
    if(!empty($students)){
        ...
        foreach($mixed_stds as $code => $student){
            ...
            array_push($result, ['sn'=>++$sn,'name'=>$student, 'gender'=>$stud['gender'],'Phone number'=>$stud['guardian_number'], 'Guardian'=>$stud['guardian']]);
        }
    }
...
//foter page
    ...
    $pdf->Cell(10,7,$lang[$_SESSION['lang']]['phonebook'].': '.$className.' - '.$Model->GetYearName($Model->GetCurrentYear()[0]['id']),0);
    $pdf->Ln();
    ...
    $pdf->Cell(29,5,$lang[$_SESSION['lang']]["GuardianPhone"],1, 0, '', true);
    $pdf->Cell(49,5,$lang[$_SESSION['lang']]["GuardianName"],1, 0, '', true);
    $pdf->SetFillColor(0,0,128);
        ...
        if($k%2==0){
            ...
            foreach($row as $key=> $column){
                if($key == 'sn'){
                    ...
                    $pdf->Cell(29,5,$column,1, 0, '', true);
                }elseif ($key == 'Guardian'){
                    $pdf->Cell(49,5,$column,1, 0, '', true);
                }
            }
        }else{
